    ...
    // PRINTING THE CONTENTS OF THE BOARD
		...
		*/
		...
		if((y != 0) && (board[y - 1][x] >= 10) && (board[y - 1][x] <= 30) && evtab[3] == coeffUP) //up
		{
			...
			players[5].score+=board[y-1][x]/10;                          //players[5].score, because we're team 6, so we're player 5
			board[y][x]=0;
			board[y-1][x]=6;
		...
		{
			score+=board[y+1][x]/10;
			players[5].score+=board[y+1][x]/10;
			board[y][x]=0;
			board[y+1][x]=6;
		...
		else if((x != (col - 1)) && (board[y][x+1] >= 10) && (board[y][x+1] <= 30) && evtab[3] == coeffRIGHT) //right
		{
			...
			players[5].score+=board[y][x+1]/10;
			board[y][x]=0;
			board[y][x+1]=6;
...
/////////
    ...
    char enter = 0;                                                             //SHAMELESSLY COPIED FROM STACKOVERFLOW. REMEMBER TO DELETE IT
    while (enter != '\r' && enter != '\n') { enter = getchar(); }               //
